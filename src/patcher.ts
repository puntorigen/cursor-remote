...
function buildPatchCode(v: DiscoveredVars): string {
  ...
  return [
    ...
    // _getConversation: read in-memory bubbles for a loaded composer
    `${CR}.registerCommand("cursorRemote._getConversation",async(n,e)=>{`,
      `try{`,
        `if(!e||!e.composerId)return{ok:false,error:"composerId required"};`,
        `var ds=n.get(${DS});`,
        `var h=null;`,
        `try{h=ds.getHandleIfLoaded(e.composerId)}catch(he){}`,
        `if(!h){try{h=ds.getHandleIfLoaded_MIGRATED(e.composerId)}catch(he){}}`,
        `if(!h){try{h=ds.getComposerHandleById(e.composerId)}catch(he){}}`,
        `if(!h){`,
          `var avail=[];`,
          `try{var ac=ds.allComposersData;if(ac&&ac.allComposers){for(var ai=0;ai<Math.min(ac.allComposers.length,5);ai++){avail.push(ac.allComposers[ai].composerId)}}}catch(ae){}`,
          `return{ok:true,loaded:false,debug:"no handle",tried:["getHandleIfLoaded","getHandleIfLoaded_MIGRATED","getComposerHandleById"],sampleIds:avail}`,
        `}`,
        `var conv=null;`,
        `try{conv=ds.getLoadedConversation(e.composerId)}catch(ce){}`,
        `if(!conv){try{var cd2=ds.getComposerData(h);if(cd2&&cd2.conversation)conv=cd2.conversation}catch(ce){}}`,
        `if(!conv||!conv.length)return{ok:true,loaded:false,debug:"no conversation",hasHandle:true};`,
        `var cd=ds.getComposerData(h);`,
        `var genIds=(cd&&cd.generatingBubbleIds)||[];`,
        `var isStr=genIds.length>0;`,
        `var bubbles=[];`,
        `for(var bi=0;bi<conv.length;bi++){`,
          `var ref=conv[bi];`,
          `var b=ds.getComposerBubble(h,ref.bubbleId);`,
          `if(!b)continue;`,
          `var tb=null;`,
          `if(b.toolFormerData){`,
            `try{tb={tool:b.toolFormerData.tool||"",params:b.toolFormerData.params,status:(b.toolFormerData.additionalData&&b.toolFormerData.additionalData.status)||"",result:b.toolFormerData.result||""}}catch(te){}`,
          `}`,
          `var cbs=[];`,
          `if(b.codeBlocks&&b.codeBlocks.length){`,
            `for(var ci=0;ci<b.codeBlocks.length;ci++){`,
              `var cb=b.codeBlocks[ci];`,
              `cbs.push({code:cb.code||"",language:cb.language||"",uri:cb.uri?String(cb.uri):""})`,
            `}`,
          `}`,
          `var btype=(typeof b.type==="string")?b.type.toLowerCase():(b.type===1?"human":"ai");`,
          `bubbles.push({`,
            `bubbleId:ref.bubbleId,`,
            `type:btype,`,
            `text:b.text||"",`,
            `codeBlocks:cbs.length?cbs:undefined,`,
            `toolCall:tb||undefined,`,
            `isGenerating:genIds.indexOf(ref.bubbleId)>=0`,
          `})`,
        `}`,
        `return{ok:true,loaded:true,isStreaming:isStr,bubbles:bubbles};`,
      `}catch(x){`,
        `return{ok:false,error:String(x)};`,
      `}`,
    `})`,
