  ...
  return [
    ...
    `${CR}.registerCommand("cursorRemote._getConversation",async(n,e)=>{`,
      `try{`,
        ...
        `var ds=n.get(${DS});`,
        `var h=ds.getHandleIfLoaded(e.composerId);`,
        `if(!h)return{ok:true,loaded:false};`,
        `var conv=ds.getLoadedConversation(e.composerId);`,
        `if(!conv)return{ok:true,loaded:false};`,
